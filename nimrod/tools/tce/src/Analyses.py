...
def optimise(mujava_result_dir, experiment_dir, included_methods, handled_mutants=0):
    ...
    for mutated_class in os.listdir(mujava_result_dir):
        ...
        #print ('  Copying ', os.path.join(mujava_result_dir,mutated_class,'original', package_structure, mutated_class_name + '.class'))
        #print ('  to ', os.path.join(experiment_classes, package_structure)) 
        shutil.copy(os.path.join(mujava_result_dir,mutated_class,'original', package_structure, mutated_class_name + '.class'), os.path.join(experiment_classes, package_structure))
...
def _executeSoot(soot_jar, experiment_dir, dest_dir, target_class, jimple = True):
    command = 'java -jar %s -cp %s -d "%s" %s-O %s' % (soot_jar, experiment_dir + ':' + rt_jar_location + ':/home/leofernandesmo/workspace/easylab/defects4j/subjects/jsoup_1_fixed/lib/commons-lang-2.4.jar', dest_dir, '-f jimple ' if jimple else '', target_class)
    # print ('executing: ', command) 
    subprocess.call(command, shell=True, stdout=open(os.devnull, 'wb'))
    
def _mydiffFiles(file1, file2):
    command = 'diff "%s" "%s"' % (file1, file2)
    return subprocess.call(command, shell=True, stdout=open(os.devnull, 'wb'))
...
def _printMutsPerOperator(mut_per_operator):
    ...
    for key in sorted(mut_per_operator):
        ...
        print ('%' + str(len(key)) + 's |'.format(str(mut_per_operator[key])))
...
def ned(mujava_result_dir, included_methods):
    ...
    for mutated_class in os.listdir(mujava_result_dir):
        ...
        original_program = os.path.join(mujava_result_dir,mutated_class,'original', package_structure, mutated_class_name + '.class')
...
def printEquivsORDupsPercentagePerOperator(muts_per_op, dupes_per_op, genProportions = True):
    ...
    for key in sorted(dupes_per_op):
        ...
        # print ('%' + str(len(key) + len(value)) + 's |'.format(key))
        # str_temp = '%' + str(len(key) + len(value)) + 's |'
        # print (str_temp.format(key))
    ...
    for key in sorted(dupes_per_op):
        ...
        # print ('%' + str(len(key) + len(value)) + 's |'.format(value))
        print (('====%' + str(len(key) + len(value)) + 's |') % value)
        # str_temp = '%' + str(len(key) + len(value)) + 's |'
        # print (str_temp.format(value))
